...
def cal_perf(prediction_file, verbose = 1):
    ...
    print len(first_matched_idexs), nr_of_images
    ...
    if verbose == 1:
        ...
        print 'Mean r:'
        print mean_r
        print 'mean inverted r:'
        print round(mean_invert_r, 3)
...
def cal_perf_t2i(prediction_file, verbose=1):
    ...
    for line in open(prediction_file):
        ...
        for i in range(len(imageids)):
            if sentid.find(imageids[i] + '.jpg') == 0:
            #if sentids[i].find(imageid[i] + '.jpg') == 0:
                hit_list.append(1)
                if flag == 1:
                    first_matched_idexs.append(i + 1)
                    flag = 0
    ...
    #printStatus(INFO, 'nr of sentences: %d' % nr_of_images)
    res = [x / nr_of_images for x in res]
    ...
    return (recall_name, recall_score, med_r, mean_r)
