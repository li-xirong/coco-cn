...
if sys.platform.startswith('linux'):
    ...
    filename = os.path.join(LIB_PATH, libdir,'libsearch.dll')
    libsearch = cdll.LoadLibrary(filename)


DFUNC_MAPPING = {'l1':0, 'l2':1, 'chi2':2, 'cosine':4}
...
def load_model(feat_dir):
    shapefile = os.path.join(feat_dir, 'shape.txt')
    nr_of_images, feat_dim = map(int, open(shapefile).readline().split())
    model_file_name = os.path.join(feat_dir, 'feature.bin')
    model = libsearch.load_model(model_file_name, feat_dim, nr_of_images)
    if not model:
        print("failed to load a search model from %s" % model_file_name)
        return None

    model = toPyModel(model)
    model.load_ids(os.path.join(feat_dir, 'id.txt'))
    model.set_distance('l2')
    print ('[%s] %dx%d' % (__file__, nr_of_images, feat_dim))
    return model
